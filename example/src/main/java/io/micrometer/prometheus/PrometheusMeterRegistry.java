...
public class PrometheusMeterRegistry extends MeterRegistry {
    ...
    protected Timer newTimer(Meter.Id id, DistributionStatisticConfig distributionStatisticConfig, PauseDetector pauseDetector) {
        ...
        applyToCollector(id, (collector) ->
                addTimerSamples(distributionStatisticConfig, collector, timer, tagValues(id), false));
        return timer;
    }

    @Override
    protected <T> Gauge newGauge(Meter.Id id, @Nullable T obj, ToDoubleFunction<T> valueFunction) {
        // Extend DefaultGauge by using one with Exemplars
    ...
    private void addTimerSamples(DistributionStatisticConfig distributionStatisticConfig, MicrometerCollector collector,
                                 PrometheusTimer timer, List<String> tagValues, boolean forLongTaskTimer) {
        collector.add(tagValues, (conventionName, tagKeys) -> {
            Stream.Builder<Collector.MetricFamilySamples.Sample> samples = Stream.builder();
    ...
    /**
     * For use with {@link Config#onMeterRegistrationFailed(BiConsumer)
     ...
     * @since 1.6.0
     */
    public io.micrometer.prometheus.PrometheusMeterRegistry throwExceptionOnRegistrationFailure() {
